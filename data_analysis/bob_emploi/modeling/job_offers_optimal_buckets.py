...
def _compute_final_buckets_table(t_buckets):
    ...
    """
    final_buckets = t_buckets.groupby(
        'group_indexes')['real_buckets'].aggregate({
            'LB': np.min, 'UP': np.max})
    final_buckets['LB'] = [x[0] for x in final_buckets.LB]
    final_buckets['UP'] = [y[1] for y in final_buckets.UP]
