...
def _find_better_names_for_cities(city_info, french_cities):
    # Try to find a cleaner version of the city name in cleaned_data.
    for city_code in city_info:
        try:
            city_info[city_code]['name'] = french_cities.loc[city_code, 'name']
        except KeyError:
            # If the city is not available in this database, we will keep the default name.
            pass
...
def _list_hiring_cities(offers_rows, min_creation_date, progress, data_folder):
    ...
    _find_better_names_for_cities(city_info, french_cities)
