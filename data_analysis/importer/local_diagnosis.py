...
from os import path
import typing
from typing import AbstractSet, Any, Iterable, Iterator, Optional, Tuple
...
def csv2dicts(
    ...
    salaries = get_salaries_imt(pcs_rome_crosswalk, imt_salaries_csv)
...
_SALARIES_COLUMNS = ['juniorSalary', 'seniorSalary', 'local_id']


def get_salaries_imt(
        pcs_rome_crosswalk: str, imt_salaries_csv: str, *,
        allowed_job_ids: Optional[set[str]] = None,
        allowed_area_ids: Optional[set[str]] = None) -> pandas.DataFrame:
    ...
    salaries_dept = cleaned_data.imt_salaries(
        filename=imt_salaries_csv, pcs_crosswalk_filename=pcs_rome_crosswalk).reset_index()
    if allowed_job_ids:
        salaries_dept = salaries_dept[salaries_dept.rome_id.isin(allowed_job_ids)]
    if allowed_area_ids:
        salaries_dept = salaries_dept[salaries_dept.departement_id.isin(allowed_area_ids)]
    salaries_dept['local_id'] = salaries_dept.departement_id.str.cat(salaries_dept.rome_id, sep=':')
    if salaries_dept.empty:
        return pandas.DataFrame([], columns=_SALARIES_COLUMNS)
    ...
    return salaries_dept[_SALARIES_COLUMNS].drop_duplicates('local_id')
