...
# TODO(cyrille): Move this file to frontend/server.
...
# Matches a list item marker for markup language, e.g. "* this is an item", "2. this is another".
_MARKUP_LIST_MARKER = re.compile(r'^\s*(\*|\d+\.)\s+.*')
...
class QuotesChecker(_AllStringFieldsChecker):
    ...
    def check_value(self, field_value: str, locale: str) -> bool:
        ...
        has_curly_quotes = '’' in field_value and locale.startswith('fr')
        has_double_double_quotes = '""' in field_value
        marked_value = field_value.replace('’', '**’**').replace('""', '**""**')
        fixed_value = field_value.replace('’', "'").replace('""', '"')
        if has_curly_quotes and has_double_double_quotes:
            error_message = 'Curly quotes ’ and double double quotes ""'
        elif has_curly_quotes:
            error_message = 'Curly quotes ’'
        elif has_double_double_quotes:
            error_message = 'Double double quotes ""'
        else:
            return True
        raise FixableValueError(f'{error_message} are not allowed', marked_value, fixed_value)
...
class SpacesChecker(_AllStringFieldsChecker):
    ...
    def check_value(self, field_value: str, locale: str) -> bool:
        ...
        marked_value = field_value
        fixed_value = field_value
        errors: list[str] = []

        if _STRIPABLE_SPACE_REGEX.search(field_value):
            marked_value = _STRIPABLE_SPACE_REGEX.sub(r'**\1**', marked_value)
            fixed_value = _STRIPABLE_SPACE_REGEX.sub('', fixed_value)
            errors.append('Extra spaces at the beginning or end')
        if '&nbsp;' in field_value:
            marked_value = marked_value.replace('&nbsp;', '**&nbsp;**')
            fixed_value = fixed_value.replace('&nbsp;', ' ')
            errors.append('&nbsp; are not allowed')
        if '\u2028' in field_value:
            marked_value = marked_value.replace('\u2028', '**\u2028**')
            fixed_value = fixed_value.replace('\u2028', '\n')
            errors.append('Unicode complex line breaks are not allowed')
        if locale.startswith('fr') and _FR_SPACE_TO_MAKE_UNBREAKABLE_REGEX.search(field_value):
            marked_value = _FR_SPACE_TO_MAKE_UNBREAKABLE_REGEX.sub(r'**\1**', marked_value)
            fixed_value = _FR_SPACE_TO_MAKE_UNBREAKABLE_REGEX.sub('\xa0', fixed_value)
            errors.append(
        ...
        if locale.startswith('en') and _EN_EXTRA_SPACE_REGEX.search(field_value):
            marked_value = _EN_EXTRA_SPACE_REGEX.sub(r'**\1**', marked_value)
            fixed_value = _EN_EXTRA_SPACE_REGEX.sub('', fixed_value)
            errors.append('Space before an English double punctuation mark.')

        if errors:
            raise FixableValueError('\n'.join(errors), marked_value, fixed_value)
    ...
    def check(self, proto: message.Message, unused_locale: str) -> bool:
        ...
        for value, path, string_format in collect_formatted_strings(proto):
            if string_format != options_pb2.SCORING_PROJECT_TEMPLATE:
                continue
            # %jobSearchLengthAtCreation must only appear with the for-active-search filter.
            if '%jobSearchLengthAtCreation' in value:
...
class PartialSentenceChecker(ValueChecker):
    ...
    def check_value(self, field_value: str, unused_locale: str) -> bool:
        ...
        if field_value[0].lower() != field_value[0]:
            errors.append('be capitalized')
            marked_value = f'**{marked_value[0]}**{marked_value[1:]}'
            fixed_value = f'{fixed_value[0].lower()}{fixed_value[1:]}'

        if field_value[-1] in {'.', '!'}:
            errors.append('end with a punctuation mark')
            marked_value = f'{marked_value[:-1]}**{marked_value[-1]}**'
            fixed_value = fixed_value[:-1].strip()

        if errors:
            raise FixableValueError(
                f'The sentence must not {" nor ".join(errors)}',
                marked_value,
                fixed_value)
        return True
...
class MarkupChecker(ValueChecker):
    ...
    def check_value(self, field_value: str, unused_locale: str) -> bool:
        ...
        for paragraph in field_value.split('\n\n'):
            ...
            # We only allow line breaks for lists: all lines except the first one should have a
            # list marker.
            lines = paragraph.split('\n')
            num_list_items = sum(1 for line in lines[1:] if _MARKUP_LIST_MARKER.match(line))
            if not num_list_items:
                raise ValueError(
            ...
            if num_list_items == len(lines) - 1:
                continue
            raise ValueError(
                'Separate list paragraphs from others, and avoid line breaks inside each list '
                f'item: {field_value}')
        return True
