...
import operator
import re
from typing import Any, Iterable, Iterator, Literal, Mapping, Optional, Tuple
...
class StrictSyntaxTemplateError(SyntaxTemplateError):
    """A syntax template error based on strict validation."""
...
def _tokenize_mustache(
        template: str, *, use_strict_syntax: bool = False) -> Iterator[Tuple[str, _TokenType]]:
    ...
    for token in tokens:
        ...
        if clean_token.startswith('{{') or clean_token.startswith('{%'):
            token_value = clean_token[2:-2]
            stripped_token_value = token_value.strip()
            if use_strict_syntax and token_value != stripped_token_value:
                raise StrictSyntaxTemplateError(f'Extra whitespaces in {clean_token}')
            if clean_token.startswith('{{'):
                yield stripped_token_value, 'expression'
                continue
            yield stripped_token_value, 'statement'
            continue

        for tag in ('{{', '}}', '{%', '%}'):
            if tag in token:
                raise SyntaxTemplateError(f'Lonely "{tag}" found in template at "{token}"')
...
class _ForNode(_Node):
    ...
    def resolve(self, context: Mapping[str, Any]) -> str:
        ...
        return ''.join(
            ''.join(
                node.resolve(context | {self._var_name: value})
...
def instantiate(
        template: str, template_vars: Optional[Mapping[str, Any]] = None, *,
        use_strict_syntax: bool = False) -> str:
    ...
    tokens = _tokenize_mustache(template, use_strict_syntax=use_strict_syntax)
    tree = _parse_tree(tokens, '')
    return ''.join(node.resolve(context) for node in tree)


def check_syntax(template: str, *, use_strict_syntax: bool = False) -> None:
