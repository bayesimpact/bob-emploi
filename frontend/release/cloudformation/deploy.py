...
class _Deployment(typing.NamedTuple):
    ...
    def from_name(deployment: str, warn_on_deprecated: bool = True) -> '_Deployment':
        ...
        try:
            ...
            return _Deployment(
                dep['deployment'], dep['region'], dep['stackId'], tuple(dep.get('subStacks', [])))

    def get_stacks(self) -> Iterator['_Stack']:
        """Create a stack representation for each template associated with this deployment."""

        yield _Stack(_Template('main_template'), self)
        for sub_stack in self.sub_stacks:
            yield _Stack(_Template(sub_stack), self)
...
class _Template:
    ...
    def __new__(cls, name: str = 'main_template') -> '_Template':
        try:
            return cls._registry[name]
        except KeyError:
            pass
        self = cls._registry[name] = super(cls, _Template).__new__(cls)
        self.name = name
    ...
    def _s3_filename(self) -> str:
        hashed = hashlib.md5(self.content.encode('utf-8')).hexdigest()
        return f'{hashed}_{self.name}.json'
...
class TemplateTestCase(unittest.TestCase):
    ...
    def test_stable_json(self) -> None:
        ...
        self.assertEqual(self.template.content.strip(), stringify(self.template.json_content))
...
class _Stack:
    ...
    def check(self, revision: str) -> None:
        """Make sure the deployed template is consistent with the one at the given revision."""

        revision_template = _run_git(
            'show', f'{revision}:{_get_git_dirname()}/{self.template.name}.json')
        ...
        TemplateTestCase().assertEqual(revision_template, deployed_template)
    ...
    def _deploy(
        ...
        try:
            ...
            if "The submitted information didn't contain changes." in reason:
                logging.warning('No actual change in the template. Ignoring')
                return None
    ...
    def upload_new_template(self, *, change_set_name: str, dry_run: bool) -> Optional[str]:
        ...
        try:
            ...
            pass
        return self._deploy(change_set_name, dry_run=dry_run)
    ...
    def upload_new_params(
        ...
        return self._deploy(change_set_name, dry_run=dry_run, update_parameters=update_parameters)

    def wait_for_update(self) -> None:
        """Wait for the deployment to be done."""

        logging.info('Deploying the changes for %s...', self.name)
        self._call_cdf('wait', 'stack-update-complete')
...
def upload(args: '_UploadArgs') -> None:
    ...
    stack_updates = {
        ...
        for stack in d.get_stacks()
        if (change_set_url := stack.upload_new_template(
...
def check(args: '_CheckArgs') -> None:
    ...
    for deployment in args.deployments:
        for stack in deployment.get_stacks():
            try:
...
def set_parameters(args: '_ParamArgs') -> None:
    ...
    stack_updates = {
        ...
        for d in args.deployments
        for stack in d.get_stacks()
        if (change_set_url := stack.upload_new_params(args.change_set_name, dry_run, {
