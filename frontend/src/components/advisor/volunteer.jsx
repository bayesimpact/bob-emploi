...
class AdviceCard extends React.Component {
  ...
  render() {
    ...
    const associations = [];
    (associationNames || []).forEach((name, index) => {
      if (index) {
        const separator = (index === associationNames.length - 1) ? ' ou' : ','
        associations.push(<span key={`or-${associations.length}`}>{separator} </span>)
      }
      associations.push(<strong key={`name-${associations.length}`}>{name}</strong>)
    })
    return <div style={{fontSize: 30}}>
      Des associations près {prefix}<strong>{cityName}</strong> comme
      {' '}{associations} pourraient avoir besoin de vos compétences.
    </div>
  }
}
