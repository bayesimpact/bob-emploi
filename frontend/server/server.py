...
def set_nps_response_comment(set_nps_request: user_pb2.SetNPSCommentRequest) -> _FlaskResponse:
    ...
    if set_nps_request.comment:
        ...
        user_url = parse.urljoin(
            flask.request.url_root, f'eval?userId={set_nps_request.user_id}')
...
def set_ffs_response_comment(
    ...
    if set_ffs_request.comment:
        user_url = parse.urljoin(flask.request.url_root, f'eval?userId={user_id}')
        comment = '\n> '.join(set_ffs_request.comment.split('\n'))
        user.give_feedback(
            feedback_pb2.Feedback(
                user_id=user_id,
                feedback=set_ffs_request.comment,
                source=feedback_pb2.PRODUCT_FEEDBACK),
            slack_text=f'[FFS] <{user_url}|{user_id}>\n> {comment}')
