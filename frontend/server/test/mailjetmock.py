...
def get_mailjet_campaign_id(custom_campaign: str) -> Optional[int]:
    """Get the fake Mailjet Campaign ID generated for a custom campaign."""

    try:
        first_message = next(
            message for message in _MOCK_SERVER.messages
            if message.properties.get('CustomCampaign') == custom_campaign)
    except StopIteration:
        return None

    return first_message.message_id + 10000
