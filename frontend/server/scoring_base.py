...
import threading
import typing
from typing import Any, Callable, Literal, Iterable, Iterator, Mapping, Optional, Pattern, Set, \
    Tuple, Union
...
class ScoringProject:
    ...
    def get_fap_modes(self, rank: Literal['first', 'second'] = 'first') \
            -> Set['job_pb2.ApplicationMode.V']:
        """Get the FAP modes from job group info."""

        application_modes = self.job_group_info().application_modes.values()
        num_rank = 0
        if rank == 'second':
            num_rank = 1
        return set(
            fap_modes.modes[num_rank].mode for fap_modes in application_modes
            if len(fap_modes.modes) > num_rank)
