...
def check_google_token(token: str, emails_regexp: Pattern[str]) -> str:
    """Ensure a token is a Google authenticated one and return the associated email address."""

    email = decode_google_id_token(token)['email']
    if not emails_regexp.search(email):
        raise i18n.TranslatableException('Adresse email "{email}" non autorisée', email=email)
    return email
...
def decode_google_id_token(token_id: str) -> dict[str, str]:
    ...
    try:
        return typing.cast(dict[str, str], id_token.verify_oauth2_token(
            token_id, google_requests.Request(), _GOOGLE_SSO_CLIENT_ID))
    except ValueError as error:
        raise i18n.TranslatableException(
            "Mauvais jeton d'authentification\xa0: {error}.", error=str(error)) from error
