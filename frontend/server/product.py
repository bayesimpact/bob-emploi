...
class _ServerConfig(TypedDict, total=False):
    ...
    # Whether someone is reading the answers to emails.
    areEmailsAnswerRead: bool
...
class _Product:
    ...
    def load_from_env(self) -> None:
        ...
        server_config_string = os.getenv('SERVER_CONFIG', r'{}')
        try:
            self._config = json.loads(server_config_string)
        except json.decoder.JSONDecodeError as error:
            raise ValueError(f'Bad value for SERVER_CONFIG env: {server_config_string}') from error
    ...
    @property
    def are_email_answers_read(self) -> bool:
        """Whether someone is reading the answers to emails."""

        return self._config.get('areEmailsAnswerRead', True)
