...
# Slack channel (for SLACK_FEEDBACK_WEBHOOK_URL) to report volunteers wanting to give feedback.s
# os.getenv('SLACK_VOLUNTEER_FEEDBACK_CHANNEL', '')
...
def delete_user(
    ...
    if user_data.user_id:
        ...
        try:
            auth_token.check_google_token(token, re.compile('@bayesimpact.org'))
...
def _update_returning_user(
        user_data: user_pb2.User, /, force_update: bool = False) -> timestamp_pb2.Timestamp:
...
def _get_slack_feedback_webhook_url(is_for_upskilling: bool = False) -> str:
    # TODO(cyrille): Remove the fallback once the feedback URL is set up.
    url = os.getenv('SLACK_FEEDBACK_WEBHOOK_URL', os.getenv('SLACK_WEBHOOK_URL')) or ''

    if not is_for_upskilling:
        return url

    # TODO(cyrille): Move the upskilling part to the jobflix blueprint/app.
    return os.getenv('UPSKILLING_SLACK_WEBHOOK_URL', url)
...
# TODO(sil): Remove these when they are not used anymore.
def volunteer_for_feedback(topic: str, email: str) -> None:
    """Ping our Slack about a user volunteering to give feedback."""

    channel = os.getenv('SLACK_VOLUNTEER_FEEDBACK_CHANNEL', '')
    url = _get_slack_feedback_webhook_url()
    if not url:
        return
    data = {'text': f'"{email}" is volunteering to give feedback on {topic}'}
    if channel:
...
def give_project_feedback(
    ...
    give_feedback(saved_feedback, users_database=_get_user_db().with_prefix(prefix))
