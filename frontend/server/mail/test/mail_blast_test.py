...
class BlastCampaignTest(unittest.TestCase):
    ...
    def test_blast_campaign(self, mock_logging: mock.MagicMock) -> None:
        ...
        mailjet_campaign_id = mailjetmock.get_mailjet_campaign_id('focus-network')
        mock_logging.assert_any_call(
            "Report for %s blast: I've sent %d emails (and got %d errors).%s", 'focus-network',
            3, 0,
            ' You can check opening and click stats on '
    ...
    @mock.patch.dict(os.environ, {
        'SERVER_CONFIG': '{"areEmailsAnswerRead": false}',
    })
