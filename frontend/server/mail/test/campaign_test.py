...
class CampaignHelperFunctionTestCase(unittest.TestCase):
    ...
    def test_status_update_link(self) -> None:
        ...
        params = '&gender=FEMININE&hl=fr%40tu&employed=True'
        self.assertRegex(
            status_update_link,
            rf'^{re.escape(base_url)}&token=\d+\.[a-f0-9]+{params}$')
    ...
    def test_status_update_link_not_employed(self) -> None:
        ...
        status_update_link = campaign.get_status_update_link(user)
        base_url = f'https://www.bob-emploi.fr/statut/mise-a-jour?user={user_id}'
        params = '&gender=FEMININE&hl=fr%40tu&employed=False'
        self.assertRegex(
            status_update_link,
            rf'^{re.escape(base_url)}&token=\d+\.[a-f0-9]+{params}$')
    ...
    def test_get_campaign_folder(self) -> None:
        ...
        self.assertEqual(
            'bob_emploi/frontend/server/mail/templates/focus-spontaneous',
            campaign.get_campaign_folder('focus-spontaneous'))
...
class CampaignIntegrationTestCase(base_test.ServerTestCase):
    ...
    def test_logged_url(self) -> None:
        ...
        response = self.app.post(
            ...
            data=json.dumps({'userId': user_id, 'authToken': auth_token}),
