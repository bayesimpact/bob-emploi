...
const NetflixPage = (): React.ReactElement => {
  ...
  const hasFeedbackBanner = config.hasFeedbackBanner
  ...
  return <div style={containerStyle}>
    ...
    {realSections.map((section, index) => {
      return <React.Fragment key={index}>
        <Section style={sectionStyle} key={section.id} {...section} />
        {hasFeedbackBanner && index === 1 ? <Section
          style={sectionStyle} key="feedback" {...section} isFeedbackBanner={true}
          jobs={undefined} /> : null}
      </React.Fragment>
    })}
