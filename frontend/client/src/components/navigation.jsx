...
class NavigationBarBase extends React.Component {
  static propTypes = {
    ...
    setMenuShown: PropTypes.func.isRequired,
    style: PropTypes.object,
  ...
  closeMenu = () => {
    const {isMenuShown, setMenuShown} = this.props
    if (!isMenuShown) {
      return
    }
    this.timeout = setTimeout(setMenuShown, 50)
  }
  ...
  renderOnMobile() {
    const {children, dispatch, isLoggedIn, isMenuShown, project, setMenuShown} = this.props
    const {isModifyProjectModalShown} = this.state
    const style = {
      alignItems: 'center',
      backgroundColor: colors.BOB_BLUE,
      color: '#fff',
    ...
    const linkStyle = page => ({
      backgroundColor: page === this.props.page ? colors.BOB_BLUE : 'inherit',
      color: page === this.props.page ? '#fff' : colors.DARK_TWO,
      display: 'block',
      padding: '20px 40px',
      textDecoration: 'none',
    })
            ...
            <OutsideClickHandler onOutsideClick={this.closeMenu}>
              <div style={menuStyle}>
                ...
                {isLoggedIn ? <React.Fragment>
                  ...
                  <a
                    style={linkStyle()}
                    onClick={() => window.open(config.helpRequestUrl, '_blank')}>
                    Nous contacter
                  </a>
  ...
  render() {
    const {dispatch, children, featuresEnabled, isLoggedIn, isMenuShown, isOnboardingComplete,
...
class TitleUpdaterBase extends React.Component {
  ...
  componentDidUpdate({location: {pathname: previousPathname}}) {
    ...
    // Update the title.
    Array.from(document.getElementsByTagName('title')).forEach(titleElement => {
      titleElement.innerText = title
    })

    // Update the description.
    Array.from(document.getElementsByTagName('meta')).forEach(metaElement => {
...
class PageWithNavigationBar extends React.Component {
  ...
  render() {
    ...
    if (isContentScrollable) {
      ...
      if (isMobileVersion && isMenuShown) {
        containerStyle.height = '100vh'
    ...
    return <div style={containerStyle}>
      ...
      <div style={{display: 'flex', flex: 1, flexDirection: 'column'}}>
        <NavigationBar
          ...
          setMenuShown={isMenuShown => this.setState({isMenuShown})} isMenuShown={isMenuShown}
          style={{flexShrink: 0}}>
          {navBarContent}
        </NavigationBar>
...
class ModifyProjectModal extends React.Component {
  ...
  render() {
    ...
    const buttonsContainerStyle = {
      display: 'flex',
      flexDirection: isMobileVersion ? 'column' : 'row', justifyContent: 'space-around',
    }
