...
class EvalPage extends React.Component {
  ...
  static getDerivedStateFromProps(
    {location: {search}, match: {params: {useCaseId}}}, {selectedPoolName}) {
    if (selectedPoolName) {
      return null
    }
  ...
  advise = () => {
    ...
    dispatch(diagnoseProject(selectedUseCase.userData, 'eval')).then(diagnostic => {
      if (!this.isUnmounting) {
  ...
  getUrlFromState() {
    ...
    if (poolName === selectedPoolName && selectedUseCaseId === useCaseId) {
      return null
    }
  ...
  selectUseCase = selectedUseCase => {
    const {evaluation, userData} = selectedUseCase || {}
    this.setState({
      advices: [],
      diagnostic: null,
