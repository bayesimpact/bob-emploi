...
import type {PictoID} from 'components/picto'
...
function getAdvicePicto(adviceId: string): PictoID {
  const module = ADVICE_MODULES[adviceId] || null
  if (module && !module.pictoName && Sentry.captureMessage && !missingPicto.has(adviceId)) {
    Sentry.captureMessage(`Picto is missing for "${adviceId}".`)
    missingPicto.add(adviceId)
  }
  return module?.pictoName || defaultPicto
}
...
(props: AdvicePictoProps): React.ReactElement => {
  ...
  const pictoName = useMemo(() => getAdvicePicto(adviceId), [adviceId])
  return <Picto name={pictoName} {...imgProps} />
}
