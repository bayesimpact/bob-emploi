...
# Checking all colors with format #123abc are in the config colors.json
...
if [[ "$EXTRA_COLORS_COUNT" != "0" ]]; then
  ...
  grep -iE $COLOR_REGEX -rh --include \*.js --include \*.jsx "$FRONTEND_FOLDER/src" | grep -v ^import
  exit 1
fi

# Checking all colors of the Colors object are used.
if ! COLORS_DIFF=$(diff -y \
  <(grep -iE '#[0-9a-f]{6}' "$FRONTEND_FOLDER/cfg/colors.json" | sed 's/":.*//;s/^ *"//') \
