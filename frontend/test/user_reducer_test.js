...
describe('user reducer', () => {
  ...
  describe('diagnoseOnboarding', () => {
    ...
    it('should update project mobility', () => {
      const action = {type: DIAGNOSE_ONBOARDING, user: {projects: [{mobility: {city: 'Lyon'}}]}}
      const oldState = {projects: [{mobility: {areaType: 'CITY'}}]}
      const newState = userReducer(oldState, action)
      expect(newState.projects[0].mobility.areaType).to.equal('CITY')
      expect(newState.projects[0].mobility.city).to.equal('Lyon')
      expect(newState.revision).to.equal(1)
    })
